...
class EditableROMGenerator:
    ...
    def get_action_animation_file(self, action_id, function_used):
        ...
        with open(f"{self.args.vanilla}", "rb") as rom:
            if function_used == "edit_action_parameters":
                ...
                if action_id >= 0xDC:
                    # Actions >= 0xDC (specials, entry poses, extra jumps, ...)
                    # are each character's own param-array extension. The
                    # shared indirection table only has a valid entry at
                    # exactly 0xDC for these - higher IDs alias/collide in it -
                    # but entries are laid out contiguously from there, so
                    # look up 0xDC's real offset once and add the delta.
                    SHARED_ACTION_ARRAY = 0xA45D8
                    rom.seek(SHARED_ACTION_ARRAY + 0xDC * 0x14)
                    param_read = int.from_bytes(
                        rom.read(2), byteorder='big')
                    base_offset = (param_read >> 6)
                    param_offset = base_offset + (action_id - 0xDC)

                    # The parent's own array may not actually have a slot
                    # this far past 0xDC (e.g. Jigglypuff's ends at 0xE9) -
                    # writing/reading past it corrupts whatever data follows
                    # in ROM, so bail out instead.
                    parent_struct_addr = int(
                        character_struct_offsets[self.character['parent']], 16)
                    rom.seek(parent_struct_addr + 0x6C)
                    parent_param_array_size = int.from_bytes(
                        rom.read(4), byteorder='big')
                    if param_offset >= parent_param_array_size:
                        return None
                else:
                    SHARED_ACTION_ARRAY = 0xA45D8
    ...
    def create_rom(self):
        ...
        with open(f'{self.temp_dir.name}/inject_files.csv', 'w', encoding='utf-8') as f:
            ...
            for (file_name, action_name, flags, func) in animation_files:
                if func == 'edit_menu_action_parameters':
                    # Leaves the parent's own CSS idle/selected/victory
                    # animations in place - overwriting them crashes
                    # character select in Training mode.
                    continue
                try:
                    ...
                    if action_file is not None and action_file != 0 and action_file <= 0x853:
                        f.write(
        ...
        # Unlock all characters. 0x147028 alone (ori $t0, $zero, 0xFFFF)
        # isn't what the character-select unlock check actually reads - the
        # real gate is 14 separate `lbu`/`lhu $reg, offset($reg)` loads of a
        # per-character unlocked-flag byte/halfword from a save-data table,
        # each inlined at its own call site. Confirmed complete and exact by
        # diffing two otherwise-identical builds (one manually patched
        # working, one not) - this is every byte that differed between them.
        UNLOCK_PATCHES = {
            0x147028: "3408FFFF",
            0x51E34: "240E00FF",
            0x52704: "240F00FF",
        ...
        with open(f"{self.args.output}", "r+b") as rom:
            for offset, patch_hex in UNLOCK_PATCHES.items():
                rom.seek(offset)
                rom.write(bytes.fromhex(patch_hex))
